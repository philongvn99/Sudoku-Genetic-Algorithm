...
def get_best_fitness(board_list):
    ...
    """
    a = [fitness_function(board) for board in board_list]
    print(a)
    return min(a)
...
def mutation_operator(board_list, removal_map):
    ...
    print(col, removal_map[row], worst_board[row][col])
...
def crossover_operator(board_list, removal_map):
    ...
    while sum(fitness_values) > 0:
        ...
        next_most_fit_idx = fitness_values.index(min(x for x in fitness_values if x > 0))#least_fit_idx if least_fit_idx != most_fit_idx else (least_fit_idx + 1) % board_size
        fitness_values[next_most_fit_idx] = 0
        
        # select a position to crossover
        vertical = randint(1, board_size-2)
        horizontal = randint(1, board_size-2)
        #col = removal_map[row].index(max(removal_map[row]))
        for row in range(0, vertical + 1):
            for col in range(0, horizontal + 1):
                n = board_list[next_most_fit_idx][row][col]
                m = board_list[most_fit_idx][row][col]
                board_list[next_most_fit_idx][row][col], board_list[most_fit_idx][row][col] = m, n
        for row in range(vertical, board_size):
            for col in range(horizontal, board_size):
                n = board_list[next_most_fit_idx][row][col]
                m = board_list[most_fit_idx][row][col]
                board_list[next_most_fit_idx][row][col], board_list[most_fit_idx][row][col] = m, n
...
if __name__ == "__main__":
    seed(123)
    n = 2 # n is meant to be the dimension of the interior boxes, i.e. the board is n^2 * n^2
    population_size = 10
    ...
    puzzle_board_list = []
    for i in range(10):
        puzzle_board_list.append(deepcopy(puzzle_board))
    for puzzleID, puzzleVal in enumerate(puzzle_board_list):
        for rowID, rowVal in enumerate(puzzleVal):
            row = rowVal
            for index, value in enumerate(row):
                if value == 0:
                    row[index] = randint(1, 9)
            puzzle_board_list[puzzleID][rowID] = row
        # pretty_print(puzzle_board_list[puzzleID])
    # for puzzle in puzzle_board_list:
    #     pretty_print(puzzle)
    time = 0
    best_fitness = -1
    #for i in range(3):
